...
func (h *HandlerV2) UpdateItem(c *gin.Context) {
	...
	if c.Param("collection") == "deals" && h.telegramNotifier != nil {
		...
		if !isTelegramDealStageUpdate(objectRequest.Data) {
			statusHttp.CustomMessage = resp.GetCustomMessage()
			return
		}
...
// isTelegramDealStageUpdate recognizes both the common stage key and the
// pipeline-specific stage fields generated by CRM pipelines.
func isTelegramDealStageUpdate(data map[string]any) bool {
	for key := range data {
		normalizedKey := strings.ToLower(strings.TrimSpace(key))
		if normalizedKey == "stage" || normalizedKey == "stage_id" || normalizedKey == "status" ||
			strings.HasPrefix(normalizedKey, "pipeline_") || strings.HasPrefix(normalizedKey, "stage_") {
			return true
		}
	}
	return false
