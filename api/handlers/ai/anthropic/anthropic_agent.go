...
func (a *AnthropicAgent) RouteRequest(ctx context.Context, in models.RouterInput) (*models.HaikuRoutingResult, error) {
	historyText := buildHistoryText(in.History)
...
func (a *AnthropicAgent) DatabaseQuery(_ context.Context, in models.DatabaseQueryInput) (*models.DatabaseActionRequest, error) {
	...
	if err = json.Unmarshal([]byte(extractJSONFromText(text)), &action); err != nil {
		return nil, fmt.Errorf("database query: parse action JSON: %w | raw=%.300s", err, text)
	}
	return &action, nil
}

func (a *AnthropicAgent) callTool(agentCfg config.AgentConfig, system string, messages []models.ChatMessage, tool models.ClaudeFunctionTool) ([]byte, models.LLMUsage, string, error) {
	wire := wireToolRequest{
		Model:      agentCfg.Model,
		MaxTokens:  agentCfg.MaxTokens,
		Messages:   messages,
		Tools:      []models.ClaudeFunctionTool{tool},
