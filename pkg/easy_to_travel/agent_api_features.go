...
func AgentApiGetFeatures(services services.ServiceManagerI, req []byte) string {
	...
	var (
		...
		apiKeyRequest                = map[string]interface{}{"api_key": appId}
		offset                   int = 0
		limit                    int = 20
		apiResponse, getListResp GetListClientApiResponse
		wg                       sync.WaitGroup
		globalErr                error
	)
	...
	go func(wg *sync.WaitGroup) {
		defer wg.Done()
		fmt.Println("apiKeyRequest:", apiKeyRequest)
	...
	if len(apiResponse.Data.Data.Response) <= 0 {
		return returnErr(401, "The request requires an user authentication.", "")
	}

	if len(getListResp.Data.Data.Response) == 0 {
		return returnErr(404, "Wrong pagination parameters.", "")
	}

	var status = cast.ToStringSlice(cast.ToStringMap(apiResponse.Data.Data.Response[0])["status"])
	...
	finalRespone.Metadata.Count = len(getListResp.Data.Data.Response)
	finalRespone.Metadata.Offset = offset
	finalRespone.Metadata.Limit = limit
	finalRespone.Metadata.Totals = getListResp.Data.Data.Count
	for _, v := range getListResp.Data.Data.Response {
		var result Result
